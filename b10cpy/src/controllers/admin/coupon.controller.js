...
const {
  ...
  class couponController {
    ...
    getAll = async (req, res, next) => {
      ...
      res.render("admin/coupon", { data, statusfilter: this.getStatusFilter(statusCounts, status), keyword, linkprefix });
  };
  ...
  console.log(status);
    ...
    updateStatus = async (req, res, next) => {
      try {
        ...
        const coupontatus = status === 'active' ? 'inactive' : 'active';
        await updateItem(id, { status: coupontatus });
        res.status(200).json({ message: 'Successfully updated status', id, status: coupontatus });
      } catch (error) {
  ...
  statusTool = async (req, res, next) => {
    ...
    if (coupontatus && (coupontatus === 'active' || coupontatus === 'inactive')) {
      for (const itemId of selectedItems) {
        await updateItem(itemId, { status: coupontatus });
      }
